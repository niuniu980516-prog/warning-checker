...
async function checkDocument(extractedData, options = {}) {
  ...
  const autoResults = await runAutoChecks(allItems, pages, fullText, docxStructure, { fund });
  ...
  if (useGemini) {
    ...
    [aiResults, visionResults] = await Promise.all([
      runAiChecks(allItems, pages, fullText, docxStructure, { fund }),
      runVisionChecks(images || []),
    ]);
  } else {
    // Ollama CPU can only handle one model at a time — must run sequentially
    aiResults = await runAiChecks(allItems, pages, fullText, docxStructure, { fund });
